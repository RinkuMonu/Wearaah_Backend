...
export const subcreateCategory = async (req, res) => {
  try {
    ...
    if (gender) {
      parsedGender = typeof gender === "string" ? JSON.parse(gender) : gender;
    }
    ...
    const exists = await SubCategory.findOne({
      $or: [{ name: name }, { displayOrder: disNumber }],
    })
...
export const getsubCategoriesOnlyIdName = async (req, res) => {
  try {
    const categories = await SubCategory.find().sort().select("name").lean();
    return res.status(200).json({
      success: true,
      categories,
    });
